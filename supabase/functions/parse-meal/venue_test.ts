...
Deno.test("the from/at pattern generalizes past the seed list", () => {
  ...
  // will never be in a seed list still serves restaurant portions.
...
Deno.test("home cooking never triggers a menu block", () => {
  // Most of what this app records. Every one of these would otherwise tell
  // the model to go looking for a menu that does not exist.
  for (
    const t of [
...
Deno.test("delivery is a fallback signal, not a competing one", () => {
  // A named place says more than the bare fact of a delivery, so the
  // delivery hint only appears when nothing was named.
...
Deno.test("empty input is inert", () => {
  ...
  assertEquals(renderVenueContext([]), "");
});

Deno.test("the rendered block leaves Claude room to ignore it", () => {
  ...
  // fabricated menu number.
  const block = renderVenueContext(detectVenues("dinner from luigi's place"));
  assertStringIncludes(block, "Only if this really is a business");
  assertStringIncludes(block, "estimate as usual");
});

Deno.test("the same dish at two chains is told apart", () => {
  // The reason the feature exists: Starbucks' grilled cheese and In-N-Out's
  // are different products with different published figures.
  assertEquals(labels("grilled cheese from starbucks"), ["Starbucks"]);
  assertEquals(labels("grilled cheese at in n out"), ["In-N-Out"]);
  const block = renderVenueContext(detectVenues("grilled cheese from starbucks"));
  assertStringIncludes(block, "Starbucks");
  // The venue has to survive into dish_name, or the two collapse into one
  // repeat chip and one alias.
  assertStringIncludes(block, "Keep the operator's name in dish_name");
  // A menu the model does not know must not produce a falsely narrow range.
  assertStringIncludes(block, "Do not narrow a range");
});
